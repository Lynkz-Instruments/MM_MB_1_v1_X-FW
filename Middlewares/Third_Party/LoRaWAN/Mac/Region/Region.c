...
/* USER CODE BEGIN 1 */
#include <stdio.h>
uint8_t SetSelectedChannels(void) {
    ChanMaskSetParams_t chanMaskSet;
    uint16_t myChannelsMask[6] = {0}; // Initialize all channels disabled

    // Enable channel 64
    myChannelsMask[0] = 0x0000;
    myChannelsMask[1] = 0x0000;
    myChannelsMask[2] = 0x0000;
    myChannelsMask[3] = 0x0000;
    myChannelsMask[4] = 0x0001; // Enable channel 64
    myChannelsMask[5] = 0x0000;

    chanMaskSet.ChannelsMaskType = CHANNELS_MASK;
    chanMaskSet.ChannelsMaskIn = myChannelsMask;

    if (RegionUS915ChanMaskSet(&chanMaskSet)) {
        printf("Channel mask set successfully!\r\n");
    } else {
