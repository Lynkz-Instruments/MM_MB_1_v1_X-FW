...
/*!
 * \brief *CMARS* Retrieves the Tx Done status
 *
 * \retval Tx Done status
 */
uint8_t LoRaMac_GetTxDoneStatus(void);
...
static void OnRadioRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr )
{
    ...
    MW_LOG(TS_ON, VLEVEL_M, "RSSI: %d\r\n", rssi );
    MW_LOG(TS_ON, VLEVEL_M, "SNR: %d\r\n", snr );
    MW_LOG(TS_ON, VLEVEL_M, "Size: %d\r\n", size );
    MW_LOG(TS_ON, VLEVEL_M, "Payload: %d\r\n", payload );
...
uint8_t LoRaMac_GetTxDoneStatus(void)
{
    return LoRaMacRadioEvents.Events.TxDone;
}