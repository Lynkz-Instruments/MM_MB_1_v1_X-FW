...
void LoRaWAN_Init(void)
{
	...
	{
		Error_Handler(ERROR_FLASH_INIT);
	}
	...
	// Set RX windows RX1 and RX2.
	LmHandlerSetRx1Delay(1000);
	LmHandlerSetRx2Delay(0);
	...
	while(HAL_GetTick() - start < 100) { 
		// wait for 500 ms max to ensure that TxDone is received and processed in the Tx callback, to avoid stopping the timer before the callback is executed.
		// Also wait for RX window.
		LmHandlerProcess();
	}
...
void LoRaWAN_DeInit(void)
{
    ...
    rslt = LmHandlerHalt();
    printf("LmHandlerHalt rslt: %d\r\n", rslt);

	/* USER CODE BEGIN DeInit_Radio */
	HAL_RADIO_DeInit();
	__HAL_PWR_CLEAR_FLAG(PWR_FLAG_WRFBUSY);

	/* USER CODE END DeInit_Radio */

	/* USER CODE END LoRaWAN_DeInit_1 */
}

void LoRaWAN_checkTxDone()
{
	uint8_t txDone = 0;
    uint8_t i = 0;
	while (txDone != 1) {
		txDone = LoRaMac_GetTxDoneStatus();
		HAL_Delay(10);
		i++;
		printf("LoRaMac_GetTxDoneStatus(): %d\r\n", txDone);
		if (i > 100) { // wait for 1 second max
			break;
		}
	}
	if (txDone == 1) {
		printf("TxDone received, exiting.\r\n");
	} else {
		printf("TxDone took too long, more than 1 second!\r\n");
	}
}
...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params)
{
	...
	if (params != NULL)
	{
		printf("RX: slot=%d rssi=%d snr=%d\r\n", params->RxSlot, params->Rssi, params->Snr);
	...
	if (params->IsMcpsIndication)
	{
		if (appData != NULL)
		{
			...
			if (appData->Buffer != NULL){
				app_comm_lora_process(appData->Port, appData->Buffer, appData->BufferSize);
			}
		}
	}
...
void SendTxData(void)
{
	...
	uint32_t start = HAL_GetTick(); 
		// wait for 500 ms max to ensure that TxDone is received and processed in the Tx callback, to avoid stopping the timer before the callback is executed.
		// Also wait for RX window.
	while(HAL_GetTick() - start < DOWNLINK_WAIT_INTERVAL) {
		LmHandlerProcess();
	}
...
void SendBeaconPayload(uint16_t *max_accel, int16_t *bma400_temperature)
{
	uint8_t i = 0;
    AppData.Port = LORAWAN_USER_BEACON_PORT;
    AppData.Buffer[i++] = (*max_accel & 0xFF);
    AppData.Buffer[i++] = ((*max_accel >> 8) & 0xFF);            // Store max_accel as 2 bytes
    AppData.Buffer[i++] = (*bma400_temperature & 0xFF);
    AppData.Buffer[i++] = ((*bma400_temperature >> 8) & 0xFF);   // Store bma400_temperature as 2 bytes
    AppData.BufferSize = i;
    printf("Payload: %02X %02X\r\n",
        AppData.Buffer[0], AppData.Buffer[1]);
    SendTxData();
}

void SendHeartBeatPayload(AppMode_t mode, struct AppConfig_s config, uint16_t temperature)
{
    AppData.Port = LORAWAN_USER_HEART_BEAT_PORT;
	AppData.Buffer[0] = mode;
    memcpy(&AppData.Buffer[1], (uint8_t *)&config, sizeof(struct AppConfig_s));
	memcpy(&AppData.Buffer[4], &temperature, sizeof(temperature));
    AppData.BufferSize = 1 + sizeof(struct AppConfig_s) + 2;
    printf("Payload: %02X %02X %02X %02X %02X \r\n",
        AppData.Buffer[0], AppData.Buffer[1], AppData.Buffer[2], AppData.Buffer[3], AppData.Buffer[4]);
    SendTxData();
}

void SendResetReason(ResetReason reset_reason){
	uint8_t i = 0;

    AppData.Port = LORAWAN_USER_RESET_REASON_PORT;

    AppData.Buffer[i++] = (uint8_t)(reset_reason & 0xFF);       // Include reset reason enum value in payload
    AppData.BufferSize = i;
    SendTxData();
}
...
void SendErrorCode(uint8_t error_code){
    ...
    AppData.Port = LORAWAN_USER_ERROR_PORT;

    AppData.Buffer[i++] = (uint8_t)(error_code & 0xFF);         // Include error code in payload
    AppData.BufferSize = i;
    SendTxData();
}
